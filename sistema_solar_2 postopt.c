...
double MomentCM(double v[N_cossos][N_dim], double m[N_cossos], int dim){
    double P = 0.0; 
    for (int cos =0; cos<N_cossos; cos++){
        P+= m[cos]*v[cos][dim];
    }
...
double MomentAngular(double r[N_cossos][N_dim], double v[N_cossos][N_dim], double m[N_cossos]){
    double L =0.0;
    for (int cos =0; cos<N_cossos; cos++){
        L+= m[cos]*(r[cos][0]*v[cos][1] -r[cos][1]*v[cos][0]);
    }
    return L;
}
...
//***********DOCUMENT - CAPÇALERA**************
    FILE *pF = fopen("sistema_solar_posicions_postopt_2D.txt","w");
    fprintf(pF, "t ");
    for (int cos =0; cos<N_cossos; cos++){
        fprintf(pF,"x_%s y_%s E_%s difE_%s, Px, Py, Lz", names[cos], names[cos], names[cos], names[cos]);
    }
    fprintf(pF, "\n%lf ", 0.0);
    DistanceMatrix(r);
    for (int cos =0; cos<N_cossos; cos++){
...
//**************RUNGE KUTTA************
    ...
    for (int i=1; i<N+1; i++){
        ...
        if (i%1440 ==0){
            ...
            for (int j =0; j<N_dim; j++){
                fprintf(pF, "%lf ", MomentCM(v, m, j));
            }
            fprintf(pF, "%lf", MomentAngular(r, v, m));
        }
    }
    t0 = clock()-t0;
