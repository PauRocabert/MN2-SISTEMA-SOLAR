...
//#define N 781*24//Nombre d'iteracions (nombre de h en 781 dies)
...
double Energia(double v[cossos][dim],double distancies3[cossos][cossos], double m[cossos], int k1){
    double E =0.0;
    for (int e =0; e<dim; e++){
        E += 0.5*M[k1]*pow(v[k1][e],2)*pow((L0/T0),2);
    }
    for (int k2 =0; k2<cossos; k2++){
        if (k2 != k1){
            E+= -6.6743*pow(10,-11)/(L0*1000)*M[k2]*M[k1]/pow(distancies3[k1][k2],1.0/3.0);
        }
    }
    return E;
}
...
double MomentAngular(double r[cossos][dim], double v[cossos][dim],double m[cossos], int e){
    ...
    for (int cos =0; cos<cossos; cos++){
        L+= M[cos]*(pow(L0,2)/T0)*(r[cos][(e+1)%3]*v[cos][(e+2)%3] -r[cos][(e+2)%3]*v[cos][(e+1)%3]);
    }
    return L;
}
...
int main(){
    clock_t t0 = clock();
    FILE *output;
    output = fopen("trajectories_magnituds_conservades_SIhores.txt", "w");
